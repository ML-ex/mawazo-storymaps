...
// Load GeoJSON for Individuals Ability to Continue Research from Home
fetch('Continue_Research_from_Home.geojson') // 
  .then(response => response.json())
  .then(data => {
    L.geoJSON(data, {
      style: function (feature) {
...
// Map 6 (Access to Career Opportunities)
var map6 = L.map('map6').setView([9.145, 40.4897], 3); // Adjust to your desired center and zoom level
L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}.png', {
  attribution: '&copy; <a href="https://carto.com/attributions">CartoDB</a>',
}).addTo(map6);
...
// Load GeoJSON for Access to Career Opportunities
fetch('affected_access_to_career.geojson') // Make sure the path is correct
  .then(response => response.json())
  .then(data => {
    L.geoJSON(data, {
      style: function (feature) {
        return {
          fillColor: getColorMap6(feature.properties.significantly_decreased),
          weight: 1,
          opacity: 1,
          color: 'white',
          dashArray: '3',
      ...
      onEachFeature: function (feature, layer) {
        layer.bindTooltip(`
          <strong>Region:</strong> ${feature.properties.Region}<br>
          <strong>%Affected Your Access to Career Opportunities:</strong> ${feature.properties['significantly_decreased']}
        `, { sticky: true });

        layer.on('click', function () {
          layer.bindPopup(`
            <strong>Region:</strong> ${feature.properties.Region}<br>
            <strong>%Affected Your Access to Career Opportunities:</strong> ${feature.properties['Yes']}
          `).openPopup();
        });
      }
    }).addTo(map6);
...
// Color function for Affected Your Access to Career Opportunities
function getColorMap6(d) {
  return d > 0.360001 ? '#08306b' :
         d > 0.326501 ? '#08519c' :
         d > 0.243201 ? '#2171b5' :
         d > 0.166701 ? '#4292c6' :
         d > 0.166700 ? '#6baed6' :
